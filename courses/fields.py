...
#if the field isn't existed yet in the object (as we could saved the object before and resaving it for some reason like updating )
        if getattr(model_instance, self.attname) is None: #self.attname is the name of the field in the model
            ...
            try:
                ...
                if self.for_fields:
                    ...
                    # the query will be the value of these fields on the instance which we work on now(which have this field(orderfield ) and will be saved)
                    # course = 1 we assume it's one in this instanse 
                    # module = 3 and so on for the title
                    query = {field: getattr(model_instance, field) for field in self.for_fields}                                                                          
                    qs = qs.filter(**query)                                
                    last_item = qs.latest(self.attname)
                    vlaue = last_item.order + 1
